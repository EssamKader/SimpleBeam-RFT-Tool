...
| A50 | **NEW** | **Crack/skin reinforcement requires BOTH faces to be detailed; otherwise it is refused, with the reason stated.** §5.1's `H_avail` spans the innermost TOP main-bar layer to the innermost BOTTOM layer (A26), so it needs a layer count and a bar type on both faces. But A42/#50's derivation rule allows main bars top **and/or** bottom, so an engineer detailing one face only — an ordinary single-span case — has no `H_avail` at all. `v0.1.0` never exposed this because `Place Crack Bars` asked for `layers_top` and `layers_btm` itself and **defaulted both to 1**, so it always had two numbers whether or not either matched the beam; that defaulting is exactly the gap #48 closed, which is what surfaced the question. **Resolution:** the Review tab reports crack bars as *not requested*, naming the missing face, rather than computing `H_avail` from an assumed layer. Chosen over measuring to the concrete face on the undetailed side (which would change what `available_height_mm` receives — a core change and a §5.1 reinterpretation, not an interface decision) and over re-asking the engineer for the absent face's layer count (which reintroduces precisely the model-disagreement A26 and #48 exist to eliminate). **A refusal that names its cause is worth more than a number derived from a guess** — the same principle as A42's no-fallback rule for bar types. Decided by the project owner 2026-09-10 | #50, #48 |

| A51 | **NEW** | **End anchorage uses the opposite face's bar type when one is SELECTED, whether or not that face is being placed.** §2.1's top anchorage `a_t` depends on `O_BTM`, the bottom bar's own diameter — but A42/#50 made the two faces independent, so an engineer may request top main bars only and leave the bottom face unplaced, at which point the formula has no `O_BTM`. **Resolution:** the diameter is taken from the selected bottom `RebarBarType` even when the bottom face is not requested. The selection is a real, explicit statement of bar size (A42 makes every diameter derive from a chosen type), and refusing to use a number the engineer has already supplied would be pedantry, not rigour. **If no bottom type is selected at all, `a_t` is REFUSED for that end and says so** — there is no diameter to use and A42 forbids inventing one; this completes the rule rather than extending it. Symmetric for `a_b` and `O_TOP`. **Deliberately different from A50, and consistent with it**: anchorage needs a DIAMETER, which a selection alone supplies, whereas §5.1's `H_avail` needs a LAYER POSITION, which needs the layer count as well — so crack bars still require both faces genuinely detailed while anchorage does not. The same as-built A7 clearance cross-check (`placed_clearance_warning`) remains unavailable when only one face is placed, since it compares two placed bars rather than two diameters, and is reported as unavailable rather than approximated. Decided by the project owner 2026-09-10 | #59, #56, #50 |
