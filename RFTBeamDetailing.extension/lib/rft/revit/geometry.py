...
def beam_section_dimensions_mm(beam, from_internal_units):
    """(b, h) in mm: b is the bounding-box extent perpendicular to the
    beam's own axis (plan width), h is the vertical (Z) extent. Read from
    the bounding box rather than a family parameter so it does not depend on
    a specific family's parameter naming -- rectangular sections only, per
    rev 2 scope (A6/§1).
    """
    ...
    horiz_perp = XYZ(-axis.Y, axis.X, 0).Normalize()
    corners = _horizontal_bbox_corners(bbox)
    proj = [horiz_perp.DotProduct(c) for c in corners]
...
def _rotation_aware_local_bbox(column):
    """The column's own local (un-rotated) plan bounding box and its
    instance transform to world space, extracted via its `GeometryInstance`
    (issue #14 review finding #5): `Element.get_BoundingBox(None)` is
    always WORLD-axis-aligned, so projecting it onto the beam axis
    overstates a rotated column's footprint (a 600x600 column at 45 deg
    measures ~849 mm that way). Returns (local_bbox, transform), or
    (None, None) if no `GeometryInstance` is found, in which case the
    caller falls back to the (rotation-unsafe) AABB.
    ...
    a structural column (vs. already-transformed `Solid`s, which some
    families/`Options` combinations return directly) has not been
    confirmed without a real model. Scope limit per issue #14 review: this
    is fixed only well enough for columns; general support-type detection
    is issue #15 (S2).
    """
    options = Options()
    for geom_obj in column.get_Geometry(options):
...
def column_width_along_axis_mm(column, axis_direction, from_internal_units):
    ...
    if local_bbox is not None and transform is not None:
        local_axis_x = axis_direction.DotProduct(transform.BasisX)
        local_axis_y = axis_direction.DotProduct(transform.BasisY)
        corners = _local_bbox_corners_xy(local_bbox)
        proj = [local_axis_x * cx + local_axis_y * cy for cx, cy in corners]
